...
export default function PayWidget() {
  ...
  async function connect() {
    ...
    if (Number(net.chainId) !== 31337) {
      throw new Error(`Wrong network: ${Number(net.chainId)}. Switch to 31337.`);
    }
  ...
  async function payEth() {
    ...
    try {
      ...
      const contract = new ethers.Contract(PAYMENTS_ADDRESS, PAYMENTS_ABI, signer);
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      ...
      <button onClick={payEth}>Pay 0.01 ETH</button>
      <button onClick={withdraw}>Withdraw 0.01 ETH (merchant only)</button>
      <div>{status}</div>
    </div>
  );
}